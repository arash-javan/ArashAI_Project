function [TestResult,TrainResult]=Naive_Bayes(X,Y,No_of_folds,i,p,o)
...
%     bc=NaiveBayes.fit(Xtrn,Ytrn);
%     yhtrn=bc.predict(Xtrn);
% Mdl = fitcnb(Xtrn,Ytrn);
Mdl = fitcnb(Xtrn,Ytrn,...
...
%     label = predict(Mdl,X)
    ...
    statsTrain = confusionmatStats(Ytrn,yhtrn);
    ...
    statsTest = confusionmatStats(Ytest,Yh1);
